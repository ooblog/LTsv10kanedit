...
kanfont_seek,kanfont_fontgrid,kanfont_gridinner,kanfont_piece,kanfont_lineseg,kanfont_gothic,kanfont_gridimage="ぱ",25,0,0,0,0,"kanfont_grid25_199.png"
...
#        LTsv_glyph_calcdelete(calc_canvas); LTsv_glyph_calcresize(calc_canvas,calculatorT=LTsv_pickdatalabel(LTsv_glyph_kanline,dictype_split),calculatorTX=0,calculatorTC=len(LTsv_pickdatalabel(LTsv_glyph_kanline,dictype_split))); LTsv_glyph_calcdrawplane(calc_canvas,calc_canvasbgcolor="white"); LTsv_draw_queue()
        LTsv_glyph_calcdelete(calc_canvas); LTsv_glyph_calcresize(calc_canvas,calculatorT=LTsv_pickdatalabel(LTsv_glyph_kanline,dictype_split),calculatorTX=0,calculatorTC=len(LTsv_pickdatalabel(LTsv_glyph_kanline,dictype_split))); LTsv_glyph_calcleave(calc_canvas);
...
def kanfont_piece_shell(window_objvoid=None,window_objptr=None):
    global kanfont_seek,kanfont_fontgrid,kanfont_gridinner,kanfont_piece,kanfont_lineseg,kanfont_gothic,kanfont_gridimage
    kanfont_piece=LTsv_widget_getnumber(kanfont_piece_check)
    kanfont_glyph_draw()
...
def kanfont_glyph_mousemotion(window_objvoid=None,window_objptr=None):
    ...
    kanfont_catchZ=LTsv_widget_getnumber(kanfont_path_scale)
    if kanfont_catchP >= 0:
        if kanfont_piece !=0 and kanfont_catchP == 0:
            glyphpiece=[0]*len(glyphnote[kanfont_catchZ])
            for catchP in range(len(glyphnote[kanfont_catchZ])//2):
                glyphpiece[catchP*2],glyphpiece[catchP*2+1]=glyphnote[kanfont_catchZ][catchP*2]-glyphnote[kanfont_catchZ][kanfont_catchP*2],glyphnote[kanfont_catchZ][catchP*2+1]-glyphnote[kanfont_catchZ][kanfont_catchP*2+1]
            for catchP in range(len(glyphnote[kanfont_catchZ])//2):
                glyphnote[kanfont_catchZ][catchP*2]=min(max(kanfont_gridX*2+glyphpiece[catchP*2],0),PSchar_ZW)
                glyphnote[kanfont_catchZ][catchP*2+1]=min(max(kanfont_gridY*2+glyphpiece[catchP*2+1],0),PSchar_ZW)
            LTsv_glyph_points2path(draw_t=glyphcode,glyphnote=glyphnote,draw_g=LTsv_global_glyphtype()[kanfont_gothic])
        else:
...
def kanfont_calculatormouseinput_enter(calculator_canvas):
    ...
    if LTsv_global_dictype()[dictype_cnt] == "幅":
        LTsv_kbdentry_wide=LTsv_intstr0x(LTsv_glyph_calcresize(calculator_canvas))
        LTsv_kbdentry_edit=LTsv_glyph_calcresize(calculator_canvas,calculatorT=str(LTsv_kbdentry_wide) if 0 < LTsv_kbdentry_wide < PSfont_ZW  else "")
    LTsv_glyph_text2path(draw_t=LTsv_chrcode(LTsv_widget_getnumber(kanfont_code_scale)),kanpath=LTsv_kbdentry_edit,draw_g=LTsv_global_dictype()[dictype_cnt])
    if LTsv_global_dictype()[dictype_cnt] in "活漫筆幅":
        kanfont_seek=LTsv_chrcode(LTsv_widget_getnumber(kanfont_code_scale))
        LTsv_glyphpath(kanfont_seek)
...
def kanfont_calculatormouseleave_shell(calculator_canvas):
    ...
    def kanfont_calculatormouseleave_kernel(window_objvoid=None,window_objptr=None):
        global kanfont_calculatorenterBF,kanfont_calculatorenterAF
        kanfont_calculatorenterAF[calculator_canvas]=LTsv_glyph_calcresize(calculator_canvas)
        if kanfont_calculatorenterAF[calculator_canvas] != kanfont_calculatorenterBF[calculator_canvas]:
            kanfont_calculatormouseinput_enter(calculator_canvas)
...
def kanfont_calculatormouseenter_shell(calculator_canvas):
    ...
    def kanfont_calculatormouseenter_kernel(window_objvoid=None,window_objptr=None):
        global kanfont_calculatorenterBF,kanfont_calculatorenterAF
        kanfont_calculatorenterBF[calculator_canvas]=LTsv_glyph_calcenter(calculator_canvas)
        kanfont_calculatorenterAF[calculator_canvas]=kanfont_calculatorenterBF[calculator_canvas]
...
#                kanpath=LTsv_glyphSVG5x10x(LTsv_pickdatalabel(kanline,typeswap))
                kanpath=LTsv_glyphSVG5x10x20x(LTsv_pickdatalabel(kanline,typeswap))
...
def kanfont_configload():
    ...
    kanfont_piece=min(max(LTsv_intstr0x(LTsv_readlinerest(kanfont_config,"piece",str(kanfont_lineseg))),0),1)
...
def kanfont_configsave_exit(window_objvoid=None,window_objptr=None):
    ...
    kanfont_config=LTsv_pushlinerest(kanfont_config,"piece",str(kanfont_piece))
...
if len(LTsv_GUI) > 0:
    ...
    kanfont_configload()
    kanfont_fontsize_entry=LTsv_global_glyphkbdH()//2;    kanfont_font_entry="{0},{1}".format(kanfont_fontname["漫"],kanfont_fontsize_entry); kanfont_label_WH=kanfont_fontsize_entry*2
    ...
    kanfont_kbd_canvas=LTsv_canvas_new(kanfont_window,widget_x=0,widget_y=0,widget_w=LTsv_global_glyphkbdW(),widget_h=kanfont_entry_H,
     event_p=kanfont_kbd_mousepress,event_m=kanfont_kbd_mousemotion,event_r=kanfont_kbd_mouserelease,event_e=kanfont_kbd_mousemotion,event_l=kanfont_kbd_mousemotion,event_w=50)
    ...
    kanfont_piece_check=LTsv_check_new(kanfont_window,widget_t="Pce",widget_x=kanfont_canvas_X+kanfont_canvas_WH*6//8,widget_y=kanfont_canvas_WH+kanfont_label_WH*1,widget_w=kanfont_canvas_WH*1//8,widget_h=kanfont_label_WH,widget_f=kanfont_font_entry,event_b=kanfont_piece_shell)
    ...
    for dictype_cnt,dictype_split in enumerate(LTsv_global_dictype()):
        LTsv_glyph_calcsetup(kanfont_dictype_canvas[dictype_cnt],calculatorX=0,calculatorY=kanfont_entry_H-LTsv_global_glyphkbdH(),calculatorW=kanfont_dictype_canvasW[dictype_cnt],calculatorH=LTsv_global_glyphkbdH(),calculatorC=kanfont_clipboard,calculatorB=kanfont_calculatormouseinput_shell(kanfont_dictype_canvas[dictype_cnt]),calculatorT="")
    ...
    LTsv_widget_setnumber(kanfont_piece_check,kanfont_piece)
